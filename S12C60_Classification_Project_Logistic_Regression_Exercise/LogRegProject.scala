...
colunmNames.foreach(x => print(x + ","))
df.head(3).foreach(x => println(x))
...
df2.printSchema
val df_for_ml = (df2.select(df2("Clicked on Ad").as("label")
...
val Array(training, test) = df_for_ml.randomSplit(Array(0.7, 0.3), seed=12345)
